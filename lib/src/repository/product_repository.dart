...
Future<Stream<Product>> getProduct(String productId) async {
  ...
  uri = uri.replace(queryParameters: {'with': 'healer;category;options;optionGroups;productReviews;productReviews.user'});
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', uri));
...
Future<Stream<Product>> getProductsOfHealer(String healerId, {List<String> categories}) async {
  Uri uri = Helper.getUri('api/products/categories');
  Map<String, dynamic> query = {
    'with': 'healer;category;options;productReviews',
    'search': 'healer_id:$healerId',
    'searchFields': 'healer_id:=',
  };
...
Future<Stream<Product>> getTrendingProductsOfHealer(String healerId) async {
  Uri uri = Helper.getUri('api/products');
  uri = uri.replace(queryParameters: {
    'with': 'category;options;productReviews',
    'search': 'healer_id:$healerId;featured:1',
    'searchFields': 'healer_id:=;featured:=',
    'searchJoin': 'and',
  });
...
Future<Stream<Product>> getFeaturedProductsOfHealer(String healerId) async {
  Uri uri = Helper.getUri('api/products');
  uri = uri.replace(queryParameters: {
    'with': 'category;options;productReviews',
    'search': 'healer_id:$healerId;featured:1',
