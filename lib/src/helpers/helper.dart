...
//        },
  ...
  static double getTotalOrderPrice(ProductConsultation productOrder) {
  ...
  static double getOrderPrice(ProductConsultation productOrder) {
    double total = productOrder.price;
    productOrder.options.forEach((option) {
      total += option.price != null ? option.price : 0;
    });
  ...
  static double getTaxOrder(Consultation order) {
    double total = 0;
    order.productConsultations.forEach((productOrder) {
      total += getTotalOrderPrice(productOrder);
    });
    return order.tax * total / 100;
  }

  static double getTotalOrdersPrice(Consultation order) {
    double total = 0;
    order.productConsultations.forEach((productOrder) {
      total += getTotalOrderPrice(productOrder);
    });
  ...
  static bool canDelivery(Healer _healer, {List<Cart> carts}) {
    bool _can = true;
    String _unit = setting.value.distanceUnit;
    double _deliveryRange = 1; //_healer.deliveryRange;
    double _distance = _healer.distance;
    ...
    if (_distance == 0 && !billingAddress.value.isUnknown()) {
      _distance = sqrt(pow(69.1 * (double.parse(_healer.latitude) - billingAddress.value.latitude), 2) +
          pow(69.1 * (billingAddress.value.longitude - double.parse(_healer.longitude)) * cos(double.parse(_healer.latitude) / 57.3), 2));
    }
    _can &= false;
    //_healer.availableForDelivery && (_distance < _deliveryRange) && !deliveryAddress.value.isUnknown();
    return _can;
  }
